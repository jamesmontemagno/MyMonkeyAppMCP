...
namespace MonkeyApp
{
    ...
    {
        private static readonly Dictionary<string, int> _monkeyAccessCount = new();
        ...
        public static Monkey GetRandomMonkey()
        {
            ...
            int index = _random.Next(monkeys.Count);
            var monkey = monkeys[index];
            if (_monkeyAccessCount.ContainsKey(monkey.Name))
                _monkeyAccessCount[monkey.Name]++;
            else
        ...
        {
            return _monkeyAccessCount.TryGetValue(name, out var count) ? count : 0;
        }

        public static Monkey? GetMonkeyByName(string name)
        {
            if (string.IsNullOrWhiteSpace(name)) return null;
            var monkey = GetMonkeys().Find(m => m.Name.Equals(name, System.StringComparison.OrdinalIgnoreCase));
            if (monkey != null)
            {
                if (_monkeyAccessCount.ContainsKey(monkey.Name))
                    _monkeyAccessCount[monkey.Name]++;
